...
namespace LosTresMosqueteros.OpenIddict;
...
public class OpenIddictDataSeedContributor : OpenIddictDataSeedContributorBase, IDataSeedContributor, ITransientDependency
{
    ...
    private async Task CreateScopesAsync()
    {
        await CreateScopesAsync(new OpenIddictScopeDescriptor 
        {
            ...
            DisplayName = "LosTresMosqueteros API", 
            Resources = { "LosTresMosqueteros" }
        });
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        var appClientId = configurationSection["LosTresMosqueteros_App:ClientId"];
        if (!appClientId.IsNullOrWhiteSpace())
        {
            var appClientRootUrl = configurationSection["LosTresMosqueteros_App:RootUrl"]?.TrimEnd('/');
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["LosTresMosqueteros_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["LosTresMosqueteros_Swagger:RootUrl"]?.TrimEnd('/');
