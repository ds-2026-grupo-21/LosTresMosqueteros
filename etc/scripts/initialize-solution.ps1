...
$jobs += Start-Job -Name "DbMigrator" -ScriptBlock {
    $ErrorActionPreference = "Stop"
    Set-Location (Join-Path $using:scriptRoot "../../src/LosTresMosqueteros.DbMigrator")
    dotnet run
    dotnet run
...
$jobs += Start-Job -Name "DevCert" -ScriptBlock {
    $ErrorActionPreference = "Stop"
    Set-Location (Join-Path $using:scriptRoot "../../src/LosTresMosqueteros.HttpApi.Host")
    dotnet dev-certs https -v -ep openiddict.pfx -p a56223bd-7894-4dd9-84cb-962aa1a0a89a
